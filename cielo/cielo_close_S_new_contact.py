...
def cielo (B_length = 200 , Brad_length = 427 , Brad = 6 , A_length = 5089 , S_length = 6833 , S_height = 210 , S_height_top = 190 , Width_WG = Width_WG , x = 0 , y = 0):
    ...
    rec = gdspy.Rectangle((path2.x + Width_WG/2 + diss_to_metal + diss_holes_wg + 100 , path2.y - S_length/2 - holes_height /2 + 15 + 500)
                          ,(path2.x + Width_WG/2 + diss_to_metal + diss_holes_wg + holes_width + 100 , path2.y - S_length/2 + holes_height /2 - 15 + 500), **ld_Silox)
    rect = rec.fillet(50)
    top_cell.add(rect)
    
    rec = gdspy.Rectangle((path2.x + Width_WG/2 + diss_to_metal + diss_holes_wg - 15 + 100 , path2.y - S_length/2 - holes_height /2 + 500)
                          ,(path2.x + Width_WG/2 + diss_to_metal + diss_holes_wg + holes_width + 15 + 100 ,path2.y - S_length/2 + holes_height /2 + 500), **ld_METAL2)
    rect = rec.fillet(50)
    top_cell.add(rect)
    ...
    path7.segment((S_height*np.cos(a2r(S_height/2)) + S_height_top*(1-np.cos(a2r(S_height/2))) + diss_to_metal*2 + Metal_width*2)/2 - 100  ,"+x",**ld_METAL2)
    ...
    path8.segment((S_height*np.cos(a2r(S_height/2)) + S_height_top*(1-np.cos(a2r(S_height/2))) + diss_to_metal*2 + Metal_width*2)/2 - 80  ,"+x",**ld_METAL2)
...
[x1 , y1 , x0 , y0] = cielo()
lib.write_gds('cielo spliter_close_new_contact.gds')