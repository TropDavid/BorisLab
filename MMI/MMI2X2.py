# -*- coding: utf-8 -*-
"""
Created on Wed May 31 10:30:58 2023
...
def MMI2X2 (B_length = 100 , Brad_length = 20 , Brad = 0.3 , MMI_length = 30 , MMI_width = 5 , diss_edge = 0.5 
    ...
    rect = gdspy.Rectangle((path2.x , Center + Width_WG/2 - MMI_width/2 ),(path2.x + MMI_length , Center + Width_WG/2 + MMI_width/2 ), **ld_NWG)
    top_cell.add(rect)
    
    path1 = gdspy.Path( width = Width_WG + Brad ,initial_point = (path2.x + MMI_length , path2.y ))
    path1.segment(length = Brad_length , direction ="+x" , final_width = Width_WG , **ld_NWG)
    #path1.segment(length = A_length , direction ="+x" , **ld_NWG)
    
    path4 = gdspy.Path( width = Width_WG + Brad ,initial_point = (path3.x + MMI_length,path3.y))
    path4.segment(length = Brad_length , direction ="+x" , final_width = Width_WG , **ld_NWG)
    #path4.segment(length = A_length , direction ="+x" , **ld_NWG)
