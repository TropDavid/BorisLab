...
def grating(period, number_of_teeth, fill_frac, width, position, direction, lda=1, sin_theta=0,
    ...
    if direction == '-x':
        return p.rotate(0.5 * np.pi, position),circle,bound
    elif direction == '+x':
        return p.rotate(-0.5 * np.pi, position),circle,bound
    elif direction == '-y':
        return p.rotate(np.pi, position),circle,bound
    else:
        return p,circle,bound
...
def MMI2x2(x0=0,y0=0,w0=0.8,MMILength=60,MMIW=9,MMISeparate=3,taperL=10, taperW=1.3,layer=1,datatype=0):
    ...
    MMICell.add([MMI,port1,port2,port3,port4])
...
def MMI2x2_test(period=0.805, number_of_teeth=20, fill_frac=0.3106, width=21.5,wGwidth=0.8, position=(0,0), lda=1.55, sin_theta=0,layer=1,
    MMILength=75,MMIW=9,MMISeparate=3,taperL=10,
    taperW=1.2,datatype=0):
...
def MMI1x2_test(period=0.805, number_of_teeth=20, fill_frac=0.3106, width=21.5,wGwidth=0.8, position=(0,0), lda=1.55, sin_theta=0,layer=1,
    MMILength=75,MMIW=9,MMISeparate=3,taperL=10,
    taperW=1.2):
...
# grating coupler test
period_array=np.linspace(0.9,1,11)
FF_array=np.linspace(0.3,0.4,11)
...
for k in range(int(len(MMIL_array))):
    for j in range(int(len(MMItw_array))):
        MMI2x2Cell.add(gdspy.CellReference(MMI2x2_test(period=0.8,fill_frac=0.33,MMIW=9,MMISeparate=3.15,MMILength=MMIL_array[k],taperW=MMItw_array[j],position=(k*600,j*700), **ld_NWG), (0, 0)))
        MMI2x2Cell.add(gdspy.CellReference(MMI2x2_test(period=0.8,fill_frac=0.33,MMIW=6,MMISeparate=2.15,MMILength=MMIL_array1[k],taperW=MMItw_array1[j],position=(k*600,(j+2)*700), **ld_NWG), (0, 0)))
