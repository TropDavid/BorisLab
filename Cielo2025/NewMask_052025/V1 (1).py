...
def Straigh (Width_WG = Width_WG,x = 0,y = 0,length_WG = length_WG , diss_to_metal = diss_to_metal,
    ...
    PadRight(top_cell,path1.x, path1.y/2)
...
def Y_splitter (cell,B_length = 100 , Brad_length = 600 , Brad = 6 , A_length = 20010 - 700 - 4500  , S_length = 4500 , S_height = 210 
                 , S_heigth_top = 190 , Width_WG = Width_WG , x = 0 , y = 0 , Metal_width = Metal_width , diss_to_metal = diss_to_metal , El_length = 14410,M=1):
    ...
    if(M==1):
        ...
        PadRight(top_cell, x = path2.x, y = path2.y - 4500)
        PadLeft(top_cell,path2.x,path2.y - 4500)
    ...
    if(M==1):
        ...
        rec = gdspy.Rectangle((x2 - Width_WG/2 - diss_to_metal - 15 , path2.y + 1000 - 4500 - holes_height/2 - 50) 
                              ,(x3 + Width_WG/2 + diss_to_metal + 15 , path2.y + 1000 - 4500 + holes_height/2 + 50)
                              , **ld_Metal)
        cell.add(rec)
        PadLeft(top_cell, x = path3.x, y = path3.y - 4500)
...
def MZ (cell,B_length = 100 , Brad_length = 400 , Brad = 6 , A_length = 20005 - 5002.5  , S_length = 4502.5 , S_height = 210 , length_WG = 40020 
                 , S_heigth_top = 190 , Width_WG = Width_WG , x = 0 , y = 0 , Metal_width = Metal_width , diss_to_metal = diss_to_metal , El_length = 14000):
    # path4 = gdspy.Path(width = Width_WG ,initial_point = (x +  gap,y))
    # path4.segment(B_length + Brad_length + S_length + A_length + 501 + 173  ,"+y" ,**ld_LN )
    #Tri(x +gap,y)
    ...
    PadRight(top_cell, x = path2.x, y = path2.y)
    PadLeft(top_cell,path2.x,path2.y)
    PadLeft(top_cell,path3.x,path3.y)
    rec = gdspy.Rectangle((path2.x - Width_WG/2 - diss_to_metal - 15 , path2.y + 1000 - holes_height/2 - 50) 
                          ,(path3.x + Width_WG/2 + diss_to_metal + 15 , path2.y + 1000 + holes_height/2 + 50)
                          , **ld_Metal)
    cell.add(rec)
...
def Splitter1X4 (cell,B_length_Y = 100 , Brad_length_Y = 300 , Brad_Y = 6 , A_length_Y = 100  , S_length_Y = 1500 , S_height_Y = 210 
                 , S_heigth_top_Y = 190 , Width_WG = Width_WG , x = 0 , y = 0 , Metal_width = Metal_width , diss_to_metal = diss_to_metal , El_length = 14410):
    ...
    [r_x,r_y,l_x,l_y] = Y_splitter(cell = cell , B_length=B_length_Y ,Brad_length=Brad_length_Y
                                   ...
                                   ,Metal_width=Metal_width,diss_to_metal=diss_to_metal,El_length=0,M=0)
        
    pathr = gdspy.Path(width=Width_WG,initial_point=(r_x,r_y))
    pathr = sbendPath(wgsbend = pathr , L = S_length_Y , H = S_heigth_top_Y*2 , info = ld_LN)
    ...
    Y_splitter(cell = cell , B_length=B_length_Y,Brad_length=Brad_length_Y
               ...
               ,Metal_width=Metal_width,diss_to_metal=diss_to_metal,El_length=40020 - pathr.y - B_length_Y - Brad_length_Y - S_length_Y*2 - 100 ,M=1)
    
    pathl = gdspy.Path(width=Width_WG,initial_point=(l_x,l_y))
    pathl.x = pathl.x - S_height_Y
    pathl = sbendPathM(wgsbend = pathl , L = S_length_Y , H = S_height_Y*2 , info = ld_LN)
    ...
    Y_splitter(cell = cell , B_length=B_length_Y,Brad_length=Brad_length_Y
               ...
               ,Metal_width=Metal_width,diss_to_metal=diss_to_metal,El_length=40020 - pathl.y - B_length_Y - Brad_length_Y - S_length_Y*2 - 100,M=1)
    cell.add([pathr,pathl])
    
Straigh()
Y_splitter(top_cell,x = 3000)
MZ(top_cell,x = 10000,y=0)
Splitter1X4(top_cell,x = 6000)
