...
def ViaAndPad(x = 0, y = 0,S = -1):
    ...
    cell.add(gdspy.Rectangle((x - 13*k,y - 2*k), (x + (8 + 7*side +6*dis + 14)*k , y + (20 + 7*side +6*dis + 70)*k ),**ld_P1R))
...
def Mod (x = 0 , y = 0 , L = 0 ,B = 0):
    ...
    pathFrontHTop.segment(length = 40 , direction= "+x" , **ld_P1P)
    ...
    cell.add([pathHTop , pathBackHTop ,pathFrontHTop  ])
    #########################################################################################################################################################################
...
xTop = Mod(pathTop.x , pathTop.y , L = 450)
pathTop.x = xTop[0]
xBottom = Mod(pathBottom.x,pathBottom.y , L = 450 , B = 1)
# pathBottom.segment(length = 400 , direction = "+x" , **ld_X1)
