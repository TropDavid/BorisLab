...
def Modulator(y = 0 , length_Start = 100):
    cell.add(gdspy.CellReference(Taper_end, (400,y) , rotation = 180))
    path1 = gdspy.Path(width = WG_Width , initial_point = (Taper_Length - Overlap_Length ,y))
    path1.segment(length = length_Start , direction = "+x" , **ld_X1)
    
    cell.add(gdspy.CellReference(MMI_1X2, (path1.x,path1.y)))
    
    pathTop = gdspy.Path(width = WG_Width , initial_point = (path1.x + MMI_Length - 10,y + 12.27))
    pathBottom = gdspy.Path(width = WG_Width , initial_point = (path1.x + MMI_Length - 10,y -12.27))
    ...
    pathtemp = gdspy.Path(width = 30 , initial_point = (Left_Electrode_x ,S_y))
    pathtemp.arc(radius = 100, initial_angle = a2r(-90), final_angle = a2r(-180))

    verticsMidle = [
        (pathtemp.x - 19.987 - 15, pathtemp.y-1),
        (pathtemp.x - 19.987 + 15, pathtemp.y -1),
        (pathtemp.x - 19.987 + 35, pathtemp.y + 100),
        (pathtemp.x - 19.987 - 35, pathtemp.y + 100)
        ]
    cell.add(gdspy.Polygon(verticsMidle,**ld_LNARFP))
    
    verticsLeft = [
        (pathtemp.x - 48 - 19.987 - 25, pathtemp.y-1),
        (pathtemp.x - 48 - 19.987 + 25, pathtemp.y - 1),
        (pathtemp.x  - 150  + 35, pathtemp.y + 100),
        (pathtemp.x  - 150 - 35, pathtemp.y + 100)
        ]
    cell.add(gdspy.Polygon(verticsLeft,**ld_LNARFP))
    ...
    cell.add(gdspy.Polygon(verticsRight,**ld_LNARFP))
    cell.add(gdspy.Rectangle((pathtemp.x - 19.987 + 35, pathtemp.y + 100), (pathtemp.x - 19.987 + 35 - 70, pathtemp.y + 100 + 70),**ld_LNARFP))
    cell.add(gdspy.Rectangle((pathtemp.x - 19.987 - 150  + 35, pathtemp.y + 100), (pathtemp.x - 19.987 - 150  + 35 - 70, pathtemp.y + 100 + 70),**ld_LNARFP))
    cell.add(gdspy.Rectangle((pathtemp.x - 19.987 + 150  + 35, pathtemp.y + 100), (pathtemp.x - 19.987 + 150  + 35 - 70, pathtemp.y + 100 + 70),**ld_LNARFP))
    
    cell.add(gdspy.Rectangle((pathtemp.x - 19.987 + 35, pathtemp.y + 100), (pathtemp.x - 19.987 + 35 - 70, pathtemp.y + 100 + 70),**ld_LNARFPAD))
    cell.add(gdspy.Rectangle((pathtemp.x - 19.987 - 150  + 35, pathtemp.y + 100), (pathtemp.x - 19.987 - 150  + 35 - 70, pathtemp.y + 100 + 70),**ld_LNARFPAD))
    cell.add(gdspy.Rectangle((pathtemp.x - 19.987 + 150  + 35, pathtemp.y + 100), (pathtemp.x - 19.987 + 150  + 35 - 70, pathtemp.y + 100 + 70),**ld_LNARFPAD))
...
Modulator(y = 0 , length_Start = 100)

Modulator(y = 250 , length_Start = 1000)

Modulator(y = 500 , length_Start = 2000)

lib.write_gds('Modulator_Array.gds')   
